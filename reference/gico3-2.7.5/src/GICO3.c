...
void GICO3(int dev)
{
  ...
  if(shortbase==1) {pthread_mutex_lock(&mutex);for(station_index=0;station_index<STATION;station_index++) fft[station_index]=fftwf_plan_dft_r2c_1d(points,(        float *)sample,fr,             FFTW_ESTIMATE);pthread_mutex_unlock(&mutex);}
  if(shortbase==0) {pthread_mutex_lock(&mutex);for(station_index=0;station_index<STATION;station_index++) fft[station_index]=fftwf_plan_dft_1d    (points,(fftwf_complex *)sample,fr,FFTW_FORWARD,FFTW_ESTIMATE);pthread_mutex_unlock(&mutex);}
  if(iq==1)      {pthread_mutex_lock(&mutex);for(station_index=0;station_index<STATION;station_index++) fft[station_index]=fftwf_plan_dft_1d    (points,(fftwf_complex *)sample,fr,FFTW_FORWARD,FFTW_ESTIMATE);pthread_mutex_unlock(&mutex);}//printf("haystack\n");}
  ...
  for(block=0;prm3[dev][0][block].offset!=0x7FFFFFFF;block++){
    for(station_index=0;station_index<STATION;station_index++){
      offset=prm3[dev][station_index][block].offset;phase0=prm3[dev][station_index][block].phase0;phase1=prm3[dev][station_index][block].phase1;phase2=prm3[dev][station_index][block].phase2;

      //      printf("offset=%08x raw_offset=%08x phase0=%08f phase1=%08f phase2=%08f\n",offset,raw_offset[page][station_index],phase0,phase1,phase2);
	    //  for(index=       0;index<points;index+=2) printf ("data %f",sample[index]);

      if(offset==0x80000000) memset(sample,0,sizeof(float)*points);
      else{
	func[station_index](&raw_data[page][station_index][offset-raw_offset[page][station_index]],sample,points,special[station_index][STREAM_INDEX].channel-1,terminal[station_index].level);
      }
	...
	if( special[station_index][STREAM_INDEX].sideband==IQ){
	  ...
	  for(index=0;index<points;index++ ){temp[2*index+0]=sample[index];  temp[2*index+1]=0.0;                                  } memcpy(sample,temp,sizeof(fftwf_complex)*points);//printf("Kashimax\n");
	}
      }//if IQ
    
      if(shortbase==0){phs=phase0;for(index=0;index<points;index++){temp[2*index+0]=sample[index]*cos_table[phs>>24];temp[2*index+1]=sample[index]*sin_table[phs>>24];phs+=phase1;} memcpy(sample,temp,sizeof(fftwf_complex)*points);}

      fftwf_execute(fft[station_index]);
      if(shortbase==1) gico3_track(fr,freq[station_index],phase0,phase2,points/2);else gico3_track(fr,freq[station_index],     0,phase2,points/2);
    }
