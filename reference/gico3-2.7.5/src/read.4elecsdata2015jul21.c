 ...
 for(index=0;(index<MAX_STATION)&&(strlen(station[index].name));index++){
  ...
  for(READ_EPOCH=process.epoch+process.skip;READ_EPOCH<process.epoch+process.length;READ_EPOCH++){
	...
	if( p1 == diff_vsi_size ){ 	
	  ...
	  //	  if( ((framenum+1) % cluster_size)==cluster_size ){  ss+=dummy;  
	  ss +=	header_size; 
	  if( ((framenum) % cluster_size)==cluster_size-1){  ss+=dummy;  
	    // 	  if( (framenum % cluster_size)==0 ){  ss+=dummy;  
	    if(framenum >= 511499){
	      for(pos=-100 ; pos<100 ; pos++){	    next_frame = ((char*)raw_buffer[page][index][ss+pos]); next_frame = next_frame & 0x00FFFFFF;
		if(message>=2 ){ fprintf(stdout,"GICO3:[level-2]VDIF[m frame =  %10d %10d %x diff= %10d]\n" ,framenum, next_frame, ((char*)raw_buffer[page][index][ss+pos]), pos);}
	      }
	    }
	  ...
	  //	  if( abs(next_frame-current_frame)>=20000){
	  
	  //	  jump=next_frame-framenum; 
	  ...
	  jump=next_frame-current_frame-1; 
	
	  if(message>=2&&index==0 && framenum%10000==0){ fprintf(stdout,"GICO3:[level-2]VDIF[framejump+ frame= %10d current= %10d next=%10d jump=%10d dif(f-n)=%10d dif(n-c)=%12d]\n" ,framenum,current_frame,next_frame,jump,framenum-current_frame,next_frame-current_frame);}
	  ...
	  if(current_frame+1 == next_frame || current_frame+1 == next_frame+200000 ){//||current_frame+2 == next_frame || current_frame+2 == next_frame+200000 ){
	    ...
	    if( next_frame > current_frame && index==0){ 
	      if(message>=2 && jump >10000){	       for(pos=-33 ; pos<33 ; pos++){	    next_frame = ((char*)raw_buffer[page][index][ss+pos]); next_frame = next_frame & 0x00FFFFFF;
		  fprintf(stdout,"GICO3:[level-2]VDIF[m frame =  %10d %10d %x diff= %10d]\n" ,framenum, next_frame, ((char*)raw_buffer[page][index][ss+pos]), pos);}
	      }
	      if(message>=2 ){ fprintf(stdout,"GICO3:[level-2]VDIF[framejump+ frame= %10d current= %10d next=%10d jump=%10d pos=%10d pos=%12d]\n" ,framenum,current_frame,next_frame,jump,pos,next_frame-current_frame);}
	      //		  fprintf(stdout,"GICO3:[level-2]VDIF[m frame =  %10d %10d %x diff= %10d]\n" ,framenum, next_frame, ((char*)raw_buffer[page][index][ss+pos]), pos);
	      pos += (jump*vsi_size);
